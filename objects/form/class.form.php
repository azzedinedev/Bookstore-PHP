...
//Include the language file for mesages
if (!class_exists('db')) {
	include(dirname(__FILE__)."lang/msg.en.php");
}
...
class form
{
	...
	**/	
	function form()
	{
		...
		$this->FORM_Editor_langauge			= "fr"; //Suffix du fichier de config [ ar OU fr }
		...
		$this->fieldSubmit 					= "submit"; 						//Name of the submit field
		$this->fieldReset 					= "reset"; 							//Name of the reset field


		/**
		 * configuration of the language code to include file of messages
		 * Load message on the seleted language
		 */
		$this->loadmessage($msgForm); 
	...
	**/
	...
	{
		$this->labelSubmit = $msg["txt_submit"];
		$this->labelReset = $msg["txt_reset"];
...
//				$mail_message.="\n--".$mail_B1B."--\n";
	...
	**/
	function addHTMLForms($field_name,$field_label,$field_type,$field_value,$field_options,$type_validation,$remarque, $field_add, $balise_add)
    {
		$add = "";
		if( !empty($field_add) ){
		...
		switch ($field_type){
			...
			case "radio":
                if( $this->is_not_editable == true ){
    				...
    				foreach($field_options as $val_select => $text_select) {
    					$input = $text_select.' <input id="'.$field_name.'" name="'.$field_name.'" type="radio" value="'.$val_select.'" ';
    					if($val_select == $field_value){ $input.= ' checked="checked"'; }
    					$input.= ' '.$add.' />'."\n";
    				}
				}
...
//  				$oFCKeditor->Config->UserFilesPath = 'media/upload/editor';
			...
			case "date":
                if( $this->is_not_editable == true ){
    				...
    				$input = '<input id="'.$field_name.'" name="'.$field_name.'" type="text" value="'.$field_value.'" '.$add.' />'.'<img src="'.$this->path_calendar_page_image.'" value="Cal" onclick="javascript: displayCalendar(document.forms[0].'.$field_name.','."'yyyy-mm-dd'".',this,true);">'."\n";
    				$this->calandarOnCall = true;
                    if( $type_validation == "" ){
	...
	**/
	function add($fieldDB,$show,$action,$field_details)
	{
		if( !in_array(false,$this->groups_to_show) ){
			...
			$this->addHTMLForms($field_details[0],$field_details[1],$field_details[2],$field_value,$field_details[4],$field_details[5],$field_details[6], $field_details[7], $field_details[8]);
				if( !is_array($fieldDB) ){
	...
	**/
	function showErrorHTMLForms($field,$type_validation,$remarque)
    {
		...
		$span_RMQ_input_start_remarque		= '<div '.$this->css_form_remarque.'>'."\n";
		$span_RMQ_input_end_remarque 		= '</div>'."\n";
		if( $this->isSubmit() ){
			$span_RMQ_input_start_error 	= '<div '.$this->css_form_remarque.'>'.'<span '.$this->css_form_remarque_error.'>'."\n";
			$span_RMQ_input_start_sucess 	= '<div '.$this->css_form_remarque.'>'.'<span '.$this->css_form_remarque_sucess.'>'."\n";
			$span_RMQ_input_end 			= '</div>'."\n".'</span>'."\n";
		}else{
			$span_RMQ_input_start_error 	= $span_RMQ_input_start_remarque.'<div>'."\n";
			$span_RMQ_input_start_sucess 	= $span_RMQ_input_start_remarque.'<div>'."\n";
			$span_RMQ_input_end 			= '</div>'."\n</div>";
		}
