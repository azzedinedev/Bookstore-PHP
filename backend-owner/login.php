...
function OnAfterLogin($userName, $connection, &$canLogin, &$errorMessage) {
    $sql = "SELECT `role` FROM `account` WHERE `login` = '".mysql_escape_string($userName)."'"; 
    
    $role = $connection->ExecScalarSQL($sql);
    ...
    if ( ($role != 'owner') and ($role != 'admin') ) {
      ...
      $errorMessage = 
        ...
        "Dear $userName, You have not a permission to access.";
